...
if(isset($_REQUEST['action']) and $_REQUEST['action']=="addDataRow"){
	...
	<tr>
		...
		<select name="wp[]" id="wp" data-size="10" required="required">
			...
			<?php
				$result	=	$db->query("SELECT wp_id,workpackage_name FROM `tbl_workpackage` WHERE 1 ORDER BY workpackage_name ASC ");
		...
		<td align="center" class="text-danger"><button type="button" data-toggle="tooltip" data-placement="right" onclick="if(confirm('Are you sure to remove?')){$(this).closest('tr').remove();}" class="btn btn-danger"><i class="fa fa-fw fa-trash-alt">Remove</i></button></td>
	</tr>
	<?php
	echo '|***|addmore';
}
